...
print('length of negative map:', len(negative_map))
...
if os.path.isfile(f'/home2/huggingface/datasets/sentence-transformer-embedding-data/{task}_queries_to_label_instructor_emb.pt'):
    print('use cached instructor embeddings')
    queries_to_label_emb = torch.load(
        f'/home2/huggingface/datasets/sentence-transformer-embedding-data/{task}_queries_to_label_instructor_emb.pt')
else:
    ...
    torch.save(queries_to_label_emb,
               f'/home2/huggingface/datasets/sentence-transformer-embedding-data/{task}_queries_to_label_instructor_emb.pt')
...
def process_domain(cur_domain):
    global domain_list_count
    if cur_domain=='':
        return ''
    if isinstance(cur_domain,list):
        assert isinstance(cur_domain[0],str),f"{type(cur_domain[0])}"
        domain_list_count += 1
...
print('domain_list_count:',domain_list_count)