...
global_count = 0
with open(f'/home2/huggingface/datasets/sentence-transformer-embedding-data/medmcqa_train.json') as f:
    for i in trange(num):
        line = f.readline()
        instance = json.loads(line)
        if not isinstance(instance['question'], str) or not isinstance(instance['exp'], str):
            continue
        # assert isinstance(instance['question'], str)
        # assert isinstance(instance['exp'], str),f"{type(instance['exp'])}\n{instance['exp']}"
        queries.append(instance['question'])
        docs.append(instance['exp'])
        doc_indices[instance['exp']].append(global_count)
        global_count += 1
