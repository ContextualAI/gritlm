...
global_count = 0
full_data = load_dataset('allenai/multi_lexsum','v20220616',cache_dir='/home2/huggingface/datasets/',split='train')
indices = list(range(len(full_data)))
random.shuffle(indices)
indices = indices[:num]
full_data = full_data.select(indices)
for instance in full_data:
    assert isinstance(instance['sources'], list)
    assert isinstance(instance['sources'][0], str)
    assert isinstance(instance['summary/long'], str)
    queries.append(instance['summary/long'])
    cur_doc = ' '.join(instance['sources'])
    docs.append(cur_doc)
