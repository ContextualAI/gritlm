...
global_count = 0
for subfile in ['ori_pqal.json','ori_pqaa.json']:
    with open(f'/home/tyu/my_root/Github_hongjin/pubmedqa/data/{subfile}') as f:
        cur_data = json.load(f)
    for _,instance in cur_data.items():
        if len(queries)>=num:
            break
        assert isinstance(instance['QUESTION'], str)
        assert isinstance(instance['CONTEXTS'], list)
        for s in instance['CONTEXTS']:
            assert isinstance(s,str)
        assert global_count==len(queries)
        assert global_count==len(docs)
        queries.append(instance['QUESTION'])
        cur_doc = ' '.join(instance['CONTEXTS'])
        docs.append(cur_doc)
        doc_indices[cur_doc].append(global_count)
        global_count += 1
print(len(queries))
...
for q,d in zip(queries,docs):
    ...
    instructions_to_write.append({
        'query': f"Represent the {default_domain}{random.choice(query_text_types)} for retrieving relevant {doc_text_type}s:",
        'pos': f"Represent the {default_domain}{doc_text_type} for retrieval:",
        'neg': f"Represent the {default_domain}{doc_text_type} for retrieval:",
    })
